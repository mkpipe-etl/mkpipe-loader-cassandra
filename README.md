...
| `replace` | Insert via Spark Cassandra Connector in `overwrite` mode (default for full). With `if_exists: append`: truncate + insert (preserves table schema) |
...
| `if_exists` | string | — | `replace` (overwrite) or `append` (preserve table, truncate+insert). Inherits from settings |
